    ...
    V: Clone + Send + Sync + Debug + 'static,
{
    ...
    pub fn put(&self, key: K, value: V) {
        ...
        // Check if we need to evict and get the key to remove
        let key_to_remove: Option<K> = if self.l1.len() >= self.l1_capacity {
            self.l1.iter().next().map(|first| first.key().clone())
        } else {
            None
        };
        
        // Evict if necessary (iterator is now dropped)
...
mod tests {
    ...
    fn test_two_tier_cache_l1_promotion() {
        ...
        // Both items should be in L1 (capacity is 2)
        assert_eq!(cache.l1_len(), 2);
        // Getting "a" should still find it in L1
        assert_eq!(cache.get(&"a".to_string()), Some(1));
        // L1 should still have both items
        assert_eq!(cache.l1_len(), 2);
    }
